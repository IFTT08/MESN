...
class Tester(object):
    ...
    def calc_logit(self, dataloader):
        ...
        model_feature = self.model[0]
        model_fc = self.model[1]
        #st1 = self.model[2]
        #st2 = self.model[3]
        #model_fcst = self.model[4]
        ...
        for i, (input, target) in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                feat_f, logit_f = model_feature(input_var, is_feat=True, preact=False)
                output, outputs_feature = model_fc(feat_f)
                finaloupt=output
                finaloupt[0]=output[0]
                finaloupt[1] = output[1]
                finaloupt[2] = output[2]
                finaloupt[3] = output[3]

                # f2 = st1(feat_f[1])
                # f2=model_fcst.scala2(f2)
                # f2 = model_fcst.scala3(f2)
                # finaloupt[2] = model_fcst.fc1(f2.view(feat_f[0].size(0), -1))
                #
                # f1 = st2(feat_f[2])
                # f1 = model_fcst.scala3(f1)
                # finaloupt[1] = model_fcst.fc1(f1.view(feat_f[0].size(0), -1))
                ...
                for b in range(n_stage):
                    _t = self.softmax(finaloupt[n_stage-b-1])
