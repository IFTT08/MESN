...
class Transform(nn.Module):
    def __init__(self,
        ...
        print("self.transformmmm",self.transform)
    ...
    def load_trans_matrix(self, param_dict):
        ...
        self.transform[0].weight.data.copy_(w)
        self.transform[0].bias.data.copy_(b)
    ...
    def _c2c(self):
        ...
        return nn.Sequential(
            nn.Conv2d(self.in_shape, int(self.in_shape), kernel_size=3, stride=1,groups=int(self.in_shape),padding=1, bias=False),
            nn.Conv2d(int(self.in_shape), self.out_shape, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(self.out_shape, affine=True),
    ...
    def get_param_dict(self):
        w = self.transform[0].weight.detach().cpu().numpy()
        b = self.transform[0].bias.detach().cpu().numpy()
        return {'w': w, 'b': b}
